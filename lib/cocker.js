...
exports.Cocker = ( function () {
    const log = console.log
        ...
        , { improve, update, doString
            , clone, circles, toArray } = require( 'bolgia' )
        , ooo = circles
        , onum = ooo.num
        , ostr = ooo.str
        , oobj = ooo.obj
            ...
            let cfg = improve( clone( opt ), sock_opt )
                ...
                , onClose = ( had_error ) => {
                    ...
                    if ( ( me.lost && ! had_error ) ||
                         ( me.attempts > rconn.trials ) || me.stop ) {
                        me.stop = false;
                        me.lost = true;
                        me.attempts = 1;
                        me.lapse = 0;
                    ...
                    delay( me.lapse ).then( () => me.run() );
                    // delay( me.lapse ).then( () => me.hunt() );
                }
                , onError = ( err ) => {
                    // catch errors
                }
    ...
    cproto.hunt = function ( sock_opt ) {
        ...
        let online = new Promise ( ( resolve, reject ) => {
            ...
            me.once( 'lost', ( addr ) =>
                reject( [ new Error( '(hunt) Promise rejected [' + toArray( addr ) + ']' ) ] ) );
    ...
    // re-connect to the socket when the connection is definitively lost
    cproto.watch = function ( sock_opt ) {
        ...
        let pat = new Promise ( ( resolve, reject ) => {
            me.once( 'lost', ( addr ) => {
                ...
                .catch( ( what ) =>
                    reject( [ new Error( '(watch) Promise rejected' ) ].concat( what ) ) );
            } );
        } );
        return pat;
    };
    ...
    cproto.prey = function ( addrs ) {
        ...
        return new Promise ( ( resolve, reject ) =>
            chainseq( addrs, ( host ) => 
                me.hunt( { address : host } ).then( ( arr ) => resolve( arr ) ) )
            .catch( ( what ) =>
                reject( [ new Error( '(prey) Promise rejected' ) ].concat( what ) ) ) );
    };
    ...
    cproto.die = function ( data, encoding ) {
        ...
        return new Promise( ( resolve ) => {
            me.once( 'lost', ( addr ) => resolve( [ addr ] ) );
