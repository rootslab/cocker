...
exports.Cocker = ( function () {
    ...
    var log = console.log,
        ...
        Cocker = function ( opt ) {
            ...
            initialDelay = ( typeof keepAlive === 'number' ) ? Math.abs( keepAlive ) : 0;
            me.setKeepAlive.apply( me, keepAlive ? [ true, initialDelay ] : [ false ] );
            /*
            ...
            var onEnd = function () {
                ...
                onClose = function ( had_error ) {
                    ...
                    if ( cfg.debug ) {
                        log( 'debug: close', had_error ? 'with error' : 'without error' );
                    }
                    /*
            ...
            if ( cfg.debug ) {
                ...
                var slice = Array.prototype.slice,
                    lfn = function ( name ) {
                        return function () {
                            log( 'debug:', name, [ Date.now() ].concat( slice.call( arguments, 0 ) ) );
                        };
                    };
    ...
    // this method is used only for easy logging. 
    cproto._read = function () {
        var me = this,
            options = me.options,
            pproto = me.constructor.super_.prototype,
            rstate = me._readableState,
            blen = rstate.length;

        pproto._read.call( me );
        if ( options.debug && blen ) {
            // rstate.buffer
            log( 'debug: readable', [ Date.now(), blen + ' bytes' ] );
        }
    };
    ...
    cproto.send = function ( msg, enc ) {
        ...
        if ( ! ok ) {
             ...
             */
            me.bark( 'slowdown', me.readyState, me.bufferSize );
        }
        return ok;
    };
