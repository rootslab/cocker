...
exports.Cocker = ( function () {
    ...
    cproto.hunt = function ( sock_opt ) {
        ...
        return new Promise( ( resolve, reject ) => {
            // log error only from the first attempt failure, skip others
            me.once( 'error', ( err ) => elog.push( Date.now(), err ) );
            // reject Promise when connection is definitively lost
            me.once( 'lost', ( addr ) => reject( elog ) );
            // resolve Promise when online
            me.once( 'online', ( addr ) => resolve( [ addr ] ) );
