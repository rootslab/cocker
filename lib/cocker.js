...
exports.Cocker = ( function () {
    var log = console.log
        ...
        , Cocker = function ( opt ) {
            ...
            me.constructor.super_.call( me, ( function () {
                ...
                // http://nodejs.org/api/net.html#net_new_net_socket_options
                return toString( cfg.path.fd ) !== ostr ? half : mix( half, cfg.path );
            } )() );
    ...
    cproto.run = function ( sock_opt ) {
        var me = this
            , upd = sock_opt && ( toString( sock_opt.connection ) === oobj )
            , sopt = update( me.options, sock_opt )
            ;

        if ( upd ) {
            configure( me, sopt.connection );
        }
        // running trial
        if ( toString( sopt.path.fd ) === ostr ) {
            // connect to a UNIX domain socket path
            me.connect( sopt.path.fd );
        } else {
            // reset fd
            sopt.path.fd = undefined;
            // connect to a tcp socket
            me.connect( sopt.address.port, sopt.address.host );
        }
    };
