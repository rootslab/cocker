...
exports.Cocker = ( function () {
            ...
            var onEnd = function () {
                ...
                onClose = function ( had_error ) {
                    ...
                    if ( had_error || ( ! me.lost ) ) {
                        interval = cfg.retryInterval * Math.pow( me.attempts, cfg.retryFactor );
                        secs =  ( interval / 1000 ).toFixed( 2 );

                        if ( attempts < cfg.attempts ) {
                            wmsg = 'current connection attempt was unsuccessful!! (' + attempts + '), retrying in ' + secs + ' secs';
                            me.bark( 'warning', wmsg );
                            // TODO use timers ?
                            /** /
                            var interval = cfg.retryInterval * Math.pow( me.attempts, cfg.retryFactor ),
                                secs =  ( interval / 1000 ).toFixed( 2 ),
                                attempts = me.attempts ? me.attempts : 1;
                            wmsg = 'current connection attempt was unsuccessful!! (' + attempts + '), retrying in ' + secs + ' secs';
                            me.bark( 'warning', wmsg );
                            /**/
                            ...
                            wmsg = 'current connection attempt was unsuccessful, maximum number of attempts (' + cfg.attempts + ') were reached.';
                            me.bark( 'warning', wmsg );
                            me.attempts = 0;
                            me.lost = true;
                            me.lapse = 0;
    ...
    cproto.run = function ( opt ) {
        ...
        me.bark( 'attempt', ++me.attempts, Date.now() );
    ...
    cproto.send = function ( msg, enc, cback ) {
        ...
        if ( ! ok ) {
             ...
             */
            me.bark( 'slowdown', me.readyState, me.bufferSize || -1 );
        }
        return ok;
    };
