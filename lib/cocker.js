...
exports.Cocker = ( function () {
    ...
    var log = console.log,
        ...
        Cocker = function ( opt ) {
            ...
            // call the net.Socket / super constructor with     
            ...
            var onEnd = function () {
                     ...
                     */
                    if ( me.lost ) {
                        me.lemit( 'info', 'connection ends.' );
                    } else {
                        me.lemit( 'warning', 'connection was closed.' );
                    }
                },
                onLost = function () {
                    me.attempts = 0;
                ...
                onConnect = function () {
                    ...
                    me.attempts = 0;
                    // emit online event
                    me.lemit( 'online', me.address() || me.options.fd, Date.now() );
                },
                onClose = function ( had_error ) {
                    ...
                    if ( had_error || ( ! me.lost ) ) {
                        if ( me.attempts++ < cfg.attempts ) {
                            ...
                            me.attempts = 0;
                            me.lost = true;
                            me.lemit( 'lost', 'connection was lost.', Date.now() );
                        }
                    }
                },
                onError = function ( err ) {
                    // bubble up a recall event, instead of error
                    me.lemit( 'error', err );
                };
            // add event listeners
            ...
            if ( me.options.debug ) {
                ...
                var slice = Array.prototype.slice,
                    lfn = function ( name ) {
                        return function () {
                            me.lemit( name, [ Date.now() ].concat( slice.call( arguments, 0 ) ) );
                        };
                    };
                me.on( 'data', lfn( 'data' ) );
    ...
    cproto.run = function ( opt ) {
        var me = this,
        cfg = ( opt ) ? Exts.mix( me.options, opt ) : me.options,
        str = null;
        ...
        me.lemit( 'info', 'connection status is ' + me.readyState + ' to ' + str );
    };

    // write to socket
    cproto.send = function () {
        var me = this,
        ...
        if ( ! ok ) {
             ...
             */
            me.lemit( 'slowdown', me.readyState, me.bufferSize || -1 );
        }
        return ok;
    };
