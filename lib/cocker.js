...
exports.Cocker = ( function () {
    ...
    cproto.hunt = function ( sock_opt ) {
        ...
        return new Promise( ( resolve, reject ) => {
            let t = 1
                ...
                ;
            me.once( 'error', ( err ) => last = err );
            me.once( 'lost', ( addr ) => reject( [ wmsg, last, addr, t ] ) );
            me.once( 'online', ( addr ) => resolve( [ addr, t ] ) );
            // connect to a UNIX domain socket path
            if ( doString( sopt.address.path ) === ostr )
                me.connect( sopt.address-path );
            // connect to a network socket
    ...
    cproto.die = function ( data, encoding ) {
        ...
        me.lost = true;
        return new Promise( ( resolve, reject ) => {
            me.once( 'error', ( err ) => reject( [ err.message, err, addr, 0 ] ) );
            me.once( 'lost', ( v ) => resolve( [ addr, 0 ] ) );
